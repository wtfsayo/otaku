import {
  ...
  import { fmtPct, fmtUSD, fmtNum, shortHex, fmtTokCompact } from './utils';
  ...
  function formatDetailedVaultView(v: MorphoVaultData, service: MorphoService): string {
    const chain = service.getChainSlug(); // mirror of market action
    const address = v.address ?? '';
    const link = address ? `https://app.morpho.org/${chain}/vault/${address}/` : '';
    ...
    return [
      ...
      `**Totals**  ·  Deposits **${amountStr}**  ·  USD **${v.totalDepositsUsd != null ? fmtUSD(v.totalDepositsUsd) : '—'}**`,
      `**APY**     ·  1D **${pctFromFraction(v.apy.daily)}**  ·  7D **${pctFromFraction(v.apy.weekly)}**  ·  30D **${pctFromFraction(v.apy.monthly)}**  ·  Year **${pctFromFraction(v.apy.yearly)}**`,
      link ? `🔗 **Open in Morpho:** ${link}` : ''
    ].filter(Boolean).join('\n');
  }
  
  
  /** Compact table for multiple vaults */
  function formatVaultTable(vaults: MorphoVaultData[]): string {
    if (!vaults.length) return 'No vaults found.';
    const header = [
      `| Vault | Asset | Total Deposits (tokens) | Total Deposits (USD) | 1D APY | 7D APY | 30D APY | Year APY |`,
      `|:--|:--:|--:|--:|--:|--:|--:|--:|`
    ];
  
    const rows = vaults.map(v => {
      const tokenAmt = fmtTokCompact(v.totalDepositsTokens, v.asset.symbol, 1);
      const usdAmt = v.totalDepositsUsd != null ? fmtUSD(v.totalDepositsUsd) : '—';
      return `| ${v.name} | ${v.asset.symbol} | ${tokenAmt} | ${usdAmt} | ${apyCell(v.apy.daily)} | ${apyCell(v.apy.weekly)} | ${apyCell(v.apy.monthly)} | ${apyCell(v.apy.yearly)} |`;
    });
