...
export async function exampleDirectServiceUsage(runtime: IAgentRuntime) {
  ...
  const importedWallet = await evmChainService.importWallet(testPrivateKey, 'ethereum');
  if (importedWallet) {
  ...
  if (ethWallet) {
    ...
    const balance = await evmChainService.getWalletBalance(ethWallet.address, 'ethereum');
    if (balance) {
...
console.log('========================');
console.log('');
console.log('This plugin provides comprehensive Ethereum and EVM wallet functionality.');
console.log('Users can create, import, and manage wallets across multiple chains using natural language.');
console.log('');
console.log('Supported chains:', [
  'Ethereum', 'Base', 'Arbitrum', 'Optimism', 'Polygon', 'Sepolia (testnet)'
].join(', '));
console.log('');
console.log('See the README.md file for complete documentation and usage examples.'); 