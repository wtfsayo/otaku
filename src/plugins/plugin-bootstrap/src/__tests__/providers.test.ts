    ...
    (mockRuntime.getEntityById as any).mockImplementation(async (id) => {
      if (id === ownerId) {
        return {
          ...
          names: ["Simple Owner"],
          metadata: {
            name: "SimpleOwnerName",
            username: "simple_owner_discord",
          },
        };
      }
      return null;
...
describe("Attachments Provider", () => {
  ...
  it("should not duplicate attachments with same ID", async () => {
    ...
    expect(result.data?.attachments?.[0]?.text).toBe(
      "Shared content with more details",
    );
    expect(result.data?.attachments?.[0]?.description).toBe(
      "Shared attachment",
    );
  });
