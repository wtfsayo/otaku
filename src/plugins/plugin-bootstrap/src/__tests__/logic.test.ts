...
describe("Message Handler Logic", () => {
  ...
  it("should process MESSAGE_RECEIVED event and save message to memory", async () => {
    ...
    const messageHandler =
      bootstrapPlugin.events?.[EventType.MESSAGE_RECEIVED]?.[0];
    ...
    mockRuntime.useModel = mock().mockImplementation((modelType, params) => {
      ...
      if (params?.prompt?.includes("should respond template")) {
        return Promise.reject(
          new Error("Test error in useModel for shouldRespond"),
        );
      }
      // Provide a default successful response for other useModel calls
      if (modelType === ModelType.TEXT_SMALL) {
        return Promise.resolve(
          JSON.stringify({ action: "RESPOND", providers: [], reasoning: "" }),
        );
      }
...
describe("Reaction Events", () => {
  ...
  it("should store reaction messages correctly", async () => {
    ...
    const reactionHandler =
      bootstrapPlugin.events?.[EventType.REACTION_RECEIVED]?.[0];
...
describe("World and Entity Events", () => {
  ...
  it("should handle ENTITY_JOINED events", async () => {
    ...
    const entityJoinedHandler =
      bootstrapPlugin.events?.[EventType.ENTITY_JOINED]?.[0];
  ...
  it("should handle ENTITY_LEFT events", async () => {
    ...
    const entityLeftHandler =
      bootstrapPlugin.events?.[EventType.ENTITY_LEFT]?.[0];
...
describe("Event Lifecycle Events", () => {
  ...
  it("should handle ACTION_STARTED events", async () => {
    ...
    const actionStartedHandler =
      bootstrapPlugin.events?.[EventType.ACTION_STARTED]?.[0];
  ...
  it("should handle ACTION_COMPLETED events", async () => {
    ...
    const actionCompletedHandler =
      bootstrapPlugin.events?.[EventType.ACTION_COMPLETED]?.[0];
  ...
  it("should handle EVALUATOR_STARTED events", async () => {
    ...
    const evaluatorStartedHandler =
      bootstrapPlugin.events?.[EventType.EVALUATOR_STARTED]?.[0];
  ...
  it("should handle EVALUATOR_COMPLETED events", async () => {
    ...
    const evaluatorCompletedHandler =
      bootstrapPlugin.events?.[EventType.EVALUATOR_COMPLETED]?.[0];
